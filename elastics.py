...
def main(input_options, libmode=False):
    ...
    for patt in np.arange(numStrainPatterns//numsteps):
        ...
        for a in range(0, numsteps):
            ...
            if a == 0:
                strain = np.array([float(line1[0]), float(line2[1]), float(line3[2]), 2 *
                                  float(line2[2]), 2*float(line1[2]), 2*float(line1[1])])
            else:
                strain = np.row_stack((strain, np.array([float(line1[0]), float(line2[1]), float(
                    line3[2]), 2*float(line2[2]), 2*float(line1[2]), 2*float(line1[1])])))

            # now get corresponding stress data from .castep
            (units, thisStress) = castep.get_stress_dotcastep(seedname +
                                                              "_cij__"+str(int(patt)+1)+"__"+str(a+1)+".castep")
        ...
        def __fit(index1, index2):
            ...
            if options.graphics:
                ...
                P.plot([strain[0, index2-1], strain[numsteps-1, index2-1]], [cijFitted *
                                                                             strain[0, index2-1]+intercept, cijFitted*strain[numsteps-1, index2-1]+intercept])
